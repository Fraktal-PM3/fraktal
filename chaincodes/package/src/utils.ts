...
export const hasActiveTransferProposal = async (
    ctx: Context,
    externalId: string,
    ownerMSP?: string,
): Promise<boolean> => {
    // Query all transfer terms to find any that reference this package
    const iterator = await ctx.stub.getStateByRange("", "")
       ...
       try {
        while (true) {
            ...
            if (!result.value || !result.value.value) continue
            const raw = result.value.value.toString()
            try {
                const data = validateJSONToTransferTerms(raw) // ensures shape is TransferTerms
                if (data.externalPackageId !== externalId) continue
                // if ownerMSP provided, ensure proposal was created by current owner
                if (ownerMSP && data.fromMSP !== ownerMSP) continue
                ...
                console.log(
                    `[hasActiveTransferProposal] Found active transfer proposal for package ${externalId}`,
                )
                return true
            } catch {
                // not TransferTerms — skip
                continue
            }
        }
    } finally {
